...
+ (NSDictionary *)paramsForSearchQuery:(NSString *)query range:(NSRange)pageRange
{
    ...
    //
    if (query) [dictionary setObject:[NSString stringWithFormat:@"%@",query] forKey:@"filter"];
    ...
    completion:(void (^)(NSArray *userList, NSError *error, NSUInteger totalResults))completion
{
    ...
    // Create the filter
    //
    MASFilter *filter = [MASFilter filterByAttribute:@"userName" containsWithinIt:value];
    ...
    completion:(void (^)(MASUser *user, NSError *error))completion
{
    ...
    // Build the PathURL
    //
    NSString *pathURL = [NSString stringWithFormat:@"%@/Users",
    ...
    completion:(void (^)(MASUser *user, NSError *error))completion
{
    ...
    // Create the filter
    //
    MASFilter *filter = [MASFilter filterByAttribute:@"objectId" isEqualTo:objectId];
    ...
    completion:(void (^)(MASUser *user, NSError *error))completion
{
    ...
    // Create the filter
    //
    MASFilter *filter = [MASFilter filterByAttribute:@"userName" isEqualTo:userName];
    ...
    completion:(void (^)(NSArray *userList, NSError *error, NSUInteger totalResults))completion
{
    ...
    // Build the PathURL
    //
    NSMutableString *pathURL = [NSMutableString stringWithFormat:@"%@/Users",
        [[MASConfiguration currentConfiguration] endpointPathForKey:MASSCIMEndPointKey]];
...
- (void)getThumbnailImageWithCompletion:(void (^)(UIImage *image, NSError *error))completion
{
    [MASUser getUserByUserName:self.userName
        includedAttributes:@[@"photos"]
        excludedAttributes:nil
        completion:^(MASUser *user, NSError *error)
    {
        if(completion) completion(user.photos[@"thumbnail"], error);
    }];
}
    ...
    completion:(void (^)(NSArray *userList, NSError *error, NSUInteger totalResults))completion
{
    ...
    // Build Parameters
    //
    NSString *usrName = [NSString stringWithFormat:@"userName co \"%@\"",
        userName];
    ...
    completion:(void (^)(MASUser *user, NSError *error))completion
{
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Name];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Active];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Addresses];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Emails];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Groups];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_PhoneNumbers];
    }
    ...
    {
        if(queryPath.length != 0) [queryPath appendString:kSCIM_QUERY_SEPARATOR];
        [queryPath appendString:kSCIM_User_Photos];
    }
    
    //
    // Build the full path URL
    //
    NSString *pathURL = [NSString stringWithFormat:@"%@/Users/%@?attributes=%@",
        [[MASConfiguration currentConfiguration] endpointPathForKey:MASSCIMEndPointKey],
        objectId,
        queryPath];
